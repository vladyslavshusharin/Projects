...
$showdata = "SELECT hodnoceni.*, ucet.Typ, ucet.Nazev, ucet.Profilovy_Obraz 
             FROM hodnoceni JOIN ucet ON hodnoceni.Ucet_ID = ucet.ID WHERE ucet.Typ = 'Firma'";
...
if(!empty($OborTag) || !empty($DruhTag) || !empty($MestoTag) || !empty($HodnoceniTag)){
  if(!empty($OborTag)){
...
<head>
    ...
    <title>FineJob - Hodnocení Zaměstnanců</title>
</head>
<body>
...
<!--------------------------------------SEARCH BAR---------------------------------------->  
  <div class="box-horizontal col-7 border-trans">
   ...
   <div class="bigbox-horizontal border-trans">
      <div class="vertical-center stick-to-left box-rounded bg-color-orange Hpixelbox-5 Vpixelbox-1">
        <h1 class="oswald-Cfont full-center text-color-black">Hodnocení zaměstnanců</h1>
      </div>
   </div>
  ...
  <div class="box-vertical border-trans">
    ...
    <?php if(isset($_SESSION['user_id'])):?>
     ...
     <a href="../Psani Hodnoceni/psat_hodnoceni_zamestnance.php" class="text-color-white bg-color-orange box-horizontal border-trans" style="height: 100%;"><p class="vertical-center oswald-Cfont">Napsat hodnocení</p></a>
     </div>
     <?php endif;?>
    <?php else:?>
     <div class="bg-color-orange box-rounded" style="min-height: 40px;"> 
     <a href="../Sign Up (Firma)/signup_firma.php" class="text-color-white bg-color-orange box-horizontal border-trans" style="height: 100%;"><p class="vertical-center oswald-Cfont">Chci napsat hodnocení</p></a> 
     </div>
     <?php endif;?>
...
<div class="bigbox-vertical Rpadding-0p1 Lpadding-0p1 border-trans">
  ...
  <div class="collapsible-container collapsible-button box-vertical border-trans">
    ...
    <?php if(isset($_SESSION['user_id']) && $_SESSION['user_id'] == $row['Ucet_ID']):?>
     <a href="edit_hodnoceni_zam.php?id=<?php echo $row['ID'];?>" class="box-horizontal bg-color-orange box-rounded text-color-white oswald-Cfont text-pageread">Editovat</a>
    <?php endif;?>

    <?php if(isset($_SESSION['user_id']) && $row['Ucet_ID'] == $_SESSION['user_id']):?>
      <form action="delete_hodnoceni_zam.php" method="POST">
       ...
       <div class="popup">
        ...
        <input class="box-horizontal Hpixelbox-1p5 bg-color-orange text-color-white text-full-center box-rounded Bmargin-0p1 oswald-Cfont weight-small" type="submit" value="Potvrdit">
